...
import cn.youth.datacollection.service.KScriptMonitorService;
...
public class ScriptMonitorController {
    ...
    @ApiOperation(value = "获取错误转换记录（分页）")
    @PostMapping("/findErrorRecordListByPage")
    Result<Object> findErrorRecordListByPage(@RequestBody QueryHelper<KScriptRecord> req) {
        PageInfo<KScriptRecord> kScriptMonitors = scriptRecordService.selectErrorList(req.getQuery(), req.getPage(), req.getRows());
        return Result.ok(kScriptMonitors);
    }
}
