...
import cn.youth.datacollection.service.DiScriptService;
...
public class DiScriptServiceImpl extends ServiceImpl<DiScriptMapper, DiScript> implements DiScriptService {
    ...
    public Result add(JSONObject json) throws Exception {
        ...
        RepositoryDirectoryInterface directory = abstractRepository.loadRepositoryDirectoryTree().findDirectory(RepositoryUtil.getObjectId(abstractRepository, json.get("dirId")));
        ObjectId id;
    ...
    public Result<List<TreeDTO<String>>> findDiScriptByCatagoryId(String id, String repId, Integer pageNum, Integer pageSize) {
        ...
        try {
            AbstractRepository abstractRepository = RepositoryUtil.getAbstractRepository(repId);
            RepositoryDirectoryInterface directory = abstractRepository.loadRepositoryDirectoryTree().findDirectory(RepositoryUtil.getObjectId(abstractRepository, id));
        ...
        size = subdirectories == null ? 0 : subdirectories.size();
        return Result.ok(subdirectories, size);
    }
