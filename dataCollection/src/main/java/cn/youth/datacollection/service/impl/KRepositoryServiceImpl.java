...
import cn.youth.datacollection.entity.DiCanstant;
...
public class KRepositoryServiceImpl extends ServiceImpl<KRepositoryMapper, KRepository> implements KRepositoryService {
    ...
    public TreeDTO<String> findRepTreeById(String id, RepositoryObjectType objectType) throws Exception {
        KRepository kRepository = repRepository.selectById(id);
        TreeDTO<String> treeDTO = new TreeDTO<>();
        if (kRepository != null) {
            // 连接资源库
            AbstractRepository repository = RepositoryUtil.getAbstractRepository(id);
            // 遍历获取资源库信息
            treeDTO = RepositoryUtil.getRepTreeList(treeDTO, repository, repository.loadRepositoryDirectoryTree().findDirectory("/"), objectType);
            //list = RepositoryUtil.getRepositoryTreeList(list, repository, repository.loadRepositoryDirectoryTree().findDirectory("/"), objectType);
        }
        return treeDTO;
    }
