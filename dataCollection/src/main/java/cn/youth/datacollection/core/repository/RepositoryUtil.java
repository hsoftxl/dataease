...
public class RepositoryUtil {
    ...
    public static AbstractRepository getAbstractRepository(String repId) throws Exception {
        ...
        AbstractRepository abstractRepository = repositoryObject.getRepository();
        if (abstractRepository instanceof KettleFileRepository) {
            KettleFileRepository fileRepository = (KettleFileRepository) abstractRepository;
            if (!fileRepository.isConnected()) {
                ...
                fileRepository.disconnect();
                fileRepository.connect(kRepository.getRepUsername(), kRepository.getRepPassword());
                repositoryObject.setRepository(fileRepository);
            }
        } else if (abstractRepository instanceof KettleDatabaseRepository) {
            //链接资源库
    ...
    /**
     * @param treeDTO    数据封装对象
     * @param repository 资源库
     * @param rdi        当前路径
     * @param objectType 脚本类型
     * @return
     */
    public static TreeDTO<String> getRepTreeList(TreeDTO<String> treeDTO, Repository repository, RepositoryDirectoryInterface rdi, RepositoryObjectType objectType) {
        if (!repository.isConnected()) {
            log.info("连接已断开");
        }
        treeDTO = transTree((RepositoryDirectory) rdi);
        //获取子集并添加到父级对象中
        try {
            treeDTO.setChildren(getElementTree(repository, rdi, objectType));
        } catch (KettleException e) {
            throw new RuntimeException(e);
        }
        int childrenSize = rdi.getChildren().size();
        if (childrenSize > 0) {
            for (int i = 0; i < childrenSize; i++) {
                RepositoryDirectory subDir = rdi.getSubdirectory(i);
                TreeDTO<String> childrenTreeDTO = new TreeDTO<>();
                childrenTreeDTO = getRepTreeList(childrenTreeDTO, repository, subDir, objectType);
                //添加子集，需先取出子集对象
                List<TreeDTO<String>> children = treeDTO.getChildren();
                children.add(childrenTreeDTO);
    ...
    public static List<TreeDTO<String>> getRepositoryTreeListTj(Repository repository, String dirPath) {
        ...
        try {
            ...
            // 获取子目录信息
            List<TreeDTO<String>> dirTree = getSubdirectories(new ArrayList<>(), rdi);
     ...
     * @return {@link List}
     */
    public static List<TreeDTO<String>> getSubdirectories(List<TreeDTO<String>> treeList, RepositoryDirectoryInterface rdi) {
        //将父级目录添加至返回数组中
        treeList.add(transTree((RepositoryDirectory) rdi));
        // 获取子目录的数量
        int subDirSize = rdi.getNrSubdirectories();
        if (subDirSize > 0) {
            for (int i = 0; i < subDirSize; i++) {
                RepositoryDirectory subDir = rdi.getSubdirectory(i);
                treeList = getSubdirectories(treeList, subDir);
            }
        }
        return treeList;
    }
    ...
    /**
     * 根据资源库类型获取ObjectId，通常数据库资源库为整形，文件资源库为字符串类型
     *
     * @param abstractRepository
     * @param object
     * @return
     */
    public static ObjectId getObjectId(AbstractRepository abstractRepository, Object object) {
        ObjectId objectId = null;
        if (abstractRepository instanceof KettleDatabaseRepository) {
            objectId = new LongObjectId(Long.parseLong(object.toString()));
        } else if (abstractRepository instanceof KettleFileRepository) {
            objectId = new StringObjectId(object.toString());
        }
        return objectId;
    }

    public static TreeDTO transTree(RepositoryDirectory subDir) {
        ...
        //需判断父级是否为空
        String pid = subDir.getParent() == null ? null : subDir.getParent().getObjectId().toString();
        tree.setPid("0".equals(pid) ? "0" : pid);
        return tree;
    }
