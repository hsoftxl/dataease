package cn.youth.datacollection.core.execute;

import cn.youth.datacollection.configuration.WebSocketServer;
import cn.youth.datacollection.core.log.KettleLogUtil;
import lombok.extern.slf4j.Slf4j;
import cn.youth.datacollection.util.CollectionUtil;
import cn.youth.datacollection.util.FileUtil;
import cn.youth.datacollection.util.JsonUtil;
...
public class TransExecute {
     ...
     * @param scriptId 任务id
     */
    private static String executeTrans(TransMeta tm, Map<String, String> params, String[] args, LogLevel logLevel, String scriptId) throws KettleException {
        ...
        KettleLoggingEventListener kettleLoggingEventListener = new KettleLoggingEventListener() {
            ...
            public void eventAdded(KettleLoggingEvent logs) {
                //日志实时输出到页面
                try {
                    log.warn("kettle实时执行日志：" + logs.getMessage().toString());
                    //log.error(logs.getMessage().toString());
                    WebSocketServer.sendInfo(logs.getMessage().toString(), scriptId);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
     ...
     * @param params       ktr需要的命名参数
     * @param logLevel     日志级别
     * @param scriptId     任务ID（用于实时任务传输）
     */
    public static String run(String fullPathName, Map<String, String> params, LogLevel logLevel, String scriptId) throws
            KettleException {
        // 通过ktr全路径名获取ktr元数据
        TransMeta tm = new TransMeta(FileUtil.replaceSeparator(fullPathName));
        // 开始执行ktr
        return executeTrans(tm, params, null, logLevel, scriptId);
    }
    ...
    public static String run(AbstractRepository rep, String transPath, String transName, String
            versionLabel, Map<String, String> params, LogLevel logLevel, String scriptId) throws KettleException {
        // 根据相对目录地址获取ktr所在目录信息
