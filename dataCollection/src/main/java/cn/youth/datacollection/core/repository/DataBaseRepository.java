...
public class DataBaseRepository implements RepositoryFactory {
    ...
    public AbstractRepository buildRepository(KRepository lRepository) {
        ...
        CustomKettleDatabaseRepository repository = new CustomKettleDatabaseRepository();
        // 配置资源库数据库连接信息
    ...
    public Properties setProperties(Properties properties, String databaseType) {
        ...
        } else if (databaseType.equals("OEACLE")) {
            //验证数据库连接的查询语句
            properties.setProperty("POOLING_validationQuery", "select 1 from dual");
        }
        /** -------------------------------------------------------------------- **/
