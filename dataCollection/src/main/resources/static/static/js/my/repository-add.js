...
//判断是创建资源库还是连接已有资源库
    ...
    var returnType = false;
    if ((data.repType == "fileRep" && data.repBasePath != "") ||
        (data.repType == "dbRep" && data.dbHost != "" && data.dbName != "" && data.dbPassword != "" && data.dbPort != "" && data.dbType != "")) {
        $.ajax({
            type: 'POST',
            async: false,
